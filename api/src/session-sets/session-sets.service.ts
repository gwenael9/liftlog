...
export class SessionSetsService {
  ...
  async update(
    ...
    if (dto.duration_min !== undefined) set.duration_min = dto.duration_min;
    if (dto.segments !== undefined) set.segments = dto.segments;
    if (dto.performed_at !== undefined)
      set.performed_at = new Date(dto.performed_at);
  ...
  async bulkUpdate(
    ...
    const existingSets =
      updateIds.length > 0
        ? await this.setsRepository.find({
            where: { id: In(updateIds), session_id: sessionId },
          })
        : [];
    ...
    for (const item of dto.updates) {
      ...
      if (item.exercise_order !== undefined)
        set.exercise_order = item.exercise_order;
      if (item.reps !== undefined) set.reps = item.reps;
      if (item.weight_kg !== undefined) set.weight_kg = item.weight_kg;
      if (item.duration_min !== undefined) set.duration_min = item.duration_min;
      if (item.segments !== undefined) set.segments = item.segments;
      if (item.performed_at !== undefined)
        set.performed_at = new Date(item.performed_at);
    ...
    for (const createDto of dto.creates ?? []) {
      const set = this.setsRepository.create({
        ...
        duration_min: createDto.duration_min ?? null,
        segments: createDto.segments ?? null,
        performed_at: createDto.performed_at
          ? new Date(createDto.performed_at)
          : null,
      });
      results.push(await this.setsRepository.save(set));
    }
